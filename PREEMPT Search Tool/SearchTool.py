...
class SearchTool:
	...
	def start(self):
		...
		while self.connected == False:
			...
			if self.connected == False:
				check = input("Not connected to Killgore_Scan Shared Drive. Please connect and press any key to restart.")
	...
	def specifier(self):
		...
		while exkeys == True:
			if crtkey == 0:
				key.append([input("Enter a keyword to search for. Or enter NONE to collect all files in the search directory: ")])
			else:
				key.append([input("Enter another keyword to search for IN SEPARATE FILENAMES. This is useful if you have multiple file types that you are searching for. Or enter NONE to continue: ")])
			if key[crtkey][0] != "NONE":
				while "NEXT" not in [x for x in key[crtkey]]:
					print("\nCurrent keywords to search for: %s\n" % key)
					key[crtkey].append(input("Enter another keyword to search for IN THE SAME FILENAME. Or enter NEXT to continue: "))
		...
		ext.append(".%s" % input("Enter a file extension to search for. Enter only the alphanumeric symbols after the '.' (i.e., 'txt', 'csv'). Or enter ALL to collect all files matching previous specifications: "))
		if ext[0] != ".ALL":
			while "NEXT" not in ext:
				ext.append(input("Enter another extension to search for. Collected files will contain EITHER extension. Or enter NEXT to continue: "))
		ext = ext[:-1]
		...
		for i, x in enumerate(self.fileslist):
			...
			if count >= 100:
				cmtp += 1
				print("%s files checked for match..." % (100*cmtp))
				count = 0
	...
	def director(self):
		...
		while self.newroot != "ALL":
			print("Search tool directed to %s" % self.root)
			subdirs = []
			actualdirs = []
			...
			if len(actualdirs) == 0:
				break
			print("Subdirectories listed below:")
			for i, x in enumerate(actualdirs):
				print("%s. %s" % (i+1, x))
				subdirs.append([i+1, x])
			subdirs.append(["ALL", None])

			self.newroot = input("Enter the number for the subdirectory you'd like to search. Or enter ALL to search all subdirectories: ")
		...
		for p, s, f in os.walk(self.root):
			for n in f:
				...
				if count >= 100:
					cmtp += 1
					print("%s files collected..." % (100*cmtp))
					count = 0
	...
	def writer(self):
		...
		if len(self.matchlist) > 0:
			copycheck = input("%s file(s) found matching specifications. Would you like to copy files? (y/n): " % len(self.matchlist))
			while copycheck not in [x for x in ['y', 'n']]:
				copycheck = input("Not a valid input. Would you like to copy files? (y/n): ")
			if copycheck == "y":
				print("Current saving directory: %s" % self.saveDir)
				doublecheck = input("Would you like to save here? (y/n): ")
				...
				if doublecheck == "y":
					...
					self.finished = input("Files saved. Task complete. Enter 'r' to restart program, 's' to respecify keywords, 'd' to redirect the search tool, and any other key to exit: ")
				else:
					self.finished = input("Change the saving directory in the source code if you'd like to save elsewhere. Task Complete. Enter 'r' to restart program, 's' to respecify keywords, 'd' to redirect the search tool, and any other key to exit: ")
			else:
				self.finished = input("Good bye. Enter 'r' to restart program, 's' to respecify keywords, 'd' to redirect the search tool, and any other key to exit: ")
		else:
			self.finished = input("0 files found matching specifications. Enter 'r' to restart program, 's' to respecify keywords, 'd' to redirect the search tool, and any other key to exit: ") 
